...
MODULE_AUTHOR("Igor Kryuchkov");
MODULE_DESCRIPTION("AK_Lab_6");
...
MODULE_PARM_DESC(count, "Count of 'Hello, World!' prints");
...
static int __init hello_init(void)
{
    if (count == 0 || (count >= 5 && count <= 10)) {
        printk(KERN_WARNING "Count is 0 or more than 5 less than 10");  
    }

    if (count > 10) {
        printk(KERN_ERR "Size error))))");
        return -EINVAL;
    }
